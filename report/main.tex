...
are presented in the appendix notebook (Figures 1-3), showing convergence patterns 
and validation performance throughout training.

\subsection{Detailed Performance Analysis}
The complete experimental results are documented in Appendix A (Jupyter Notebook), which 
includes training history plots, confusion matrices, and classification reports for each 
model. The Custom CNN training history (Appendix Figure 1) shows steady convergence over 
10 epochs, with validation accuracy reaching 89.8\% and final test accuracy of 90.3\%. 
The confusion matrix reveals balanced performance: 7,266 true positives (Real) and 6,520 
true positives (AI-Generated), with relatively low false positive (795) and false negative 
(690) rates.

ResNet50 and EfficientNet-B2 training curves (Appendix Figures 2-3) show faster initial 
convergence due to pretrained weights, but plateaued at lower final accuracies. The model 
comparison bar chart (Appendix Figure 4) visually demonstrates the 10\% performance gap 
between the Custom CNN and the second-best model.
...
images. The confusion matrices (Appendix Figures) reveal that the Custom CNN achieved better 
...
and approximately 2-3 minutes per epoch, achieving steady improvement as shown in the training 
history curves (Appendix Figure 1). Transfer learning models trained for only 5 epochs, with 
...
performance. The model comparison bar chart (Appendix Figure 4) clearly illustrates the 
performance gap between the three approaches.
